# Como Criar Usuário Super Admin e Inserir Módulos
...
User::create([
    ...
    'password' => Hash::make('SuaSenhaAqui123!'),
    'role' => 'super_admin',
...
```bash
php artisan tinker --execute="use App\Models\User; use Illuminate\Support\Facades\Hash; User::create(['name' => 'Super Admin', 'email' => 'admin@clivus.com.br', 'password' => Hash::make('SenhaSegura123!'), 'role' => 'super_admin', 'email_verified_at' => now()]);"
```
...
## Alterar senha de um super admin:
...
$admin->update(['password' => Hash::make('NovaSenha123!')]);
```

---

## 2. Inserir Todos os Módulos na Aplicação

### Opção A: Via Seeder (Recomendado)

Execute o seeder de módulos:
...
echo "Módulos inseridos com sucesso!\n";
echo "Total: " . Module::count() . " módulos\n";
```

### Opção C: Via Comando Artisan (Mais Simples)

```bash
php artisan db:seed --class=ModuleSeeder
```

## Verificar Módulos Inseridos
...
Module::where('category', 'tools')->get();
Module::where('category', 'finance')->get();
Module::where('category', 'management')->get();
```
